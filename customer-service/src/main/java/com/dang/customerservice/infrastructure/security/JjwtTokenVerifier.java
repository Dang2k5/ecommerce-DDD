...
public class JjwtTokenVerifier implements JwtTokenVerifier {
    ...
    public JjwtTokenVerifier(@Value("${jwt.signerKey}") String signerKey,
                             @Value("${jwt.issuer:identity-service}") String issuer) {
        byte[] bytes = signerKey.getBytes(StandardCharsets.UTF_8);
        if (bytes.length < 64) throw new IllegalArgumentException("jwt.signerKey too short for HS512. Need >= 64 chars.");
        ...
        if (roles.isEmpty()) {
            ...
            if (scope != null && !scope.isBlank()) {
                ...
                for (String a : scope.split("\\s+")) {
                    if (a != null && !a.isBlank() && a.startsWith("ROLE_")) tmp.add(a.trim());
                }
